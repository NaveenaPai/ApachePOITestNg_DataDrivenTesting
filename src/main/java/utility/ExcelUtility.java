...
public class ExcelUtility {
	...
	public static Object[][] ReadDataFromExcel(String filePath1, String sheetName1) throws Exception {
		...
		// Get the workbook from the file path specified
		...
		testData = new Object[rowsCount - 1][cellCount];
		...
		for (int i = 1; i < rowsCount; i++) // Start from i=1 to skip header row
		{
			...
			for (int j = 0; j < cellCount; j++) {
				...
				case NUMERIC: // Numeric or date field
					testData[i-1][j] = DateUtil.isCellDateFormatted(cell) ?  dateformat.format( cell.getDateCellValue()) : cell.getNumericCellValue();
					break;
				case BOOLEAN:
		...
		while (rows.hasNext()) {
			...
			while (cells.hasNext()) {
				...
				case STRING:
					...
					break;
				case NUMERIC: // Numeric or date field
					testData[rowNumber - 1][cellNumber]  = DateUtil.isCellDateFormatted(cell) ?  dateformat.format( cell.getDateCellValue())	: cell.getNumericCellValue();
					break;
				case BOOLEAN:
